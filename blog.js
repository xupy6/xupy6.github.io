...
function renderPostList(posts) {
  ...
  blogList.innerHTML = posts
    .map(
      (post, index) => `
        <button class="blog-card" type="button" data-index="${index}">
          <span>${escapeHtml(post.date || "Note")}</span>
  ...
  blogList.querySelectorAll(".blog-card").forEach((card) => {
    card.addEventListener("click", () => openPost(Number(card.dataset.index)));
  });
}
...
function openPost(index) {
  ...
  blogPost.innerHTML = `
    <header class="blog-post-head">
      ...
      <p class="eyebrow">${escapeHtml(post.date || "Blog")}</p>
